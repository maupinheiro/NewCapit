...
namespace DAL
{
    public class ConCargas
    {
        ...
        {
            // alterado a query para verifica a coluna exclusao para itens excluídos            
            string sql = "SELECT id, carga, cva, codorigem, cliorigem, cidorigem, ufcliorigem, cod_expedidor, expedidor, cid_expedidor, uf_expedidor, coddestino, clidestino, ciddestino, ufclidestino, cod_recebedor, recebedor, cid_recebedor, uf_recebedor,cod_consignatario, consignatario, cid_consignatario, uf_consignatario, cod_pagador, pagador, cid_pagador, uf_pagador, data_hora, deslocamento, distancia, emitepedagio, duracao,solicitante, gr, entrega, material, rota_entrega,status_pernoite, centro_custo_solicitacao,conta_debito_solicitacao, tipo_solicitacao, tipo_geracao_solicitacao, tipo_veiculo_solicitacao, ot, cva, disponivel_solicitacao, data_hora_coleta, idviagem,   atendimento,tipo_viagem,rota,veiculo,quant_palet,peso,pedidos,solicitacoes,estudo_rota,remessa,gate_origem, idpedagio, valorpedagio, historicopedagio, creditopedagio, pagadorpedagioida, pagadorpedagiovolta, dtemissaopedagio, doc_pedagio, cnpj_remetente, cnpj_expedidor, cnpj_destinatario, cnpj_recebedor, cnpj_consignatario, cnpj_pagador,local_pernoite, inicio_pernoite, fim_pernoite, duracao_pernoite, desc_material, gate_destino,prev_chegada,status,chegadaorigem,saidaorigem,tempoagcarreg,chegadadestino,entradaplanta,saidaplanta,tempodentroplanta, tempoesperagate,previsao, conta_debito_solicitacao, centro_custo_solicitacao, desc_veic_vw, emissao,tempoesperagate,previsao,tempoagdescarreg, duracao_transp, rede,catraca,observacao,mdfe,(select num_documento from tbnfse where idviagem=carga) as num_nfse,num_sm, percurso, valor_total, previsao_inicio_krona, previsao_termino_krona,usu_envio_krona, rota_krona,id_rota_krona FROM tbcargas WHERE fl_exclusao is null and idviagem=@idviagem order by saidaorigem";
