...
namespace NewCapit
{
    public partial class ConfereArquivo : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                if (Session["DataTable"] == null)
                {
                    ...
                    dtDados.Columns.Add("InicioJornada", Type.GetType("System.String"));
                    dtDados.Columns.Add("SaidaAlmoco", Type.GetType("System.String"));
                    dtDados.Columns.Add("RetornoAlmoco", Type.GetType("System.String"));
                    dtDados.Columns.Add("FimJornada", Type.GetType("System.String"));
        ...
        public void Txt5e()
        {
            ...
            string sql1 = "SELECT * FROM (SELECT id, codmot, nommot, DENSE_RANK() OVER (ORDER BY id DESC) AS NUMBER FROM tbmotoristas WHERE ISNUMERIC(codmot)=1 ";
            ...
            sql1 += " ) AS TBL WHERE NUMBER BETWEEN (("+PaginaAtual+"-1)*1)+1 AND ("+PaginaAtual+"*1) ORDER BY codmot DESC";
            ...
            string sqlt = "select codmot, nommot  from tbmotoristas where ISNUMERIC(codmot)=1 ";
            ...
            lblPaginaInfo.Text = $"Motorista {PaginaAtual + 1} de {totalMotoristas}";
                                            ...
                                            if (a >= 0)
                                            {
                                                if (dt4.Rows[a][2].ToString() == "PARADA CLIENTE / FORNECEDOR")
                                                {
                                                    ...
                                                    if (dt4.Rows[a][5].ToString() == "14")
                                                    {
                                                        ...
                                                        dtRow["FimJornada"] = DateTime.Parse(hora).AddMinutes(1).ToString("HH:mm").Replace(":", "");
                                ...
                                if (dtv.Rows.Count > 0)
                                {
                                    if (dtv.Rows[0][0].ToString() == "PARADA PERNOITE" || dtv.Rows[0][0].ToString() == "FIM DE JORNADA")
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            string sqli = "select top 1  hr_posicao from tb_parada where cod_cracha=" + lblCod.Text + " and ds_macro = 'REINICIO DE VIAGEM' and dt_posicao_parada='" + DateTime.Parse(dte.Rows[f][0].ToString()).ToString("yyyy-MM-dd") + "' and fl_deletado is null order by hr_posicao";
                                            SqlDataAdapter adtpi = new SqlDataAdapter(sqli, con);
                                            DataTable dti = new DataTable();
                                            con.Open();
                                            adtpi.Fill(dti);
