...
namespace NewCapit
{
    public partial class Ultima_loc : System.Web.UI.Page
    {
        ...
        //    try
        //    {
        //        if (Page.Session["usuario"].ToString() != string.Empty)
        //        {
        //            string cod_usuario = Page.Session["cod_usuario"].ToString();

        //            string sql = "select dt_ultimo_acesso, nm_nome from tb_usuario where cod_usuario=" + cod_usuario + "";
        //            SqlDataAdapter apt = new SqlDataAdapter(sql, con);
        //            DataTable dt = new DataTable();
        //            con.Open();
        //            apt.Fill(dt);
        //            con.Close();
        //            // lblAcesso.Text = "Ultimo acesso: " + DateTime.Parse(dt.Rows[0][0].ToString()).ToString("dd/MM/yyyy ") + DateTime.Parse(dt.Rows[0][0].ToString()).ToShortTimeString();
        ...
        protected void lnkCarregamento_Click(object sender, EventArgs e)
        {
            Response.Redirect("criar_carregamento.aspx");
        }
